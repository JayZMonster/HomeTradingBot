...
import requests
from utils.test_bot.settings import apis
...
class Wallet:
    ...
    def summary(self):
        ...
        summary = f'\n#Сводка по #{self.ticker[:-4]}:\n'\
                  ...
                  f'Профит: ${self.asset - (self._start_bank * self.SAFETY_CONST)}\n'\
                  f'Профит за вычетом комиссий: ${self.asset - (self._start_bank * self.SAFETY_CONST) - self.total_fee}\n' \
                  f'Уплачено комиссий: ${self.total_fee}\n' \
                  f'Доход(%): {(100 * (self.asset - self.total_fee) / (self._start_bank * self.SAFETY_CONST)) - 100}%'
        return summary
