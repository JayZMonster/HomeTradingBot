...
from utils.main_bot import settings, bot_api as api
from utils.main_bot.signals import Signal
from utils.main_bot.wallet import Wallet
from utils.main_bot.exceptions import *
...
class StochBot:
    ...
    def work(self):
        ...
        :return:
        """
        self.wallet._notify(f'\nБот - {self.ticker} работает! \n')
        try:
            self.init_test_net()
            ...
            while True:
                ...
                self.active_price = klines.Close.iloc[-1]
                # Создание новой ветки сделок
                if klines.Buy.iloc[-1] and self.active_deal is False:
                ...
                if klines.Sell.iloc[-1] and self.active_deal:
